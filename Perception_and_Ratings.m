...
for n = 2:4
    ...
    b = bar(1:8,mean(ratings(:,:,n)));
    hold on;
    e = errorbar(1:8,mean(ratings(:,:,n)),std(ratings(:,:,n))./sqrt(size(ratings,1)),'k.');
...
try
    for n = 1:size(scr_bars,3)
        subplot(3,3,3+n)
        line([0 9],repmat(mean(scr_bars(:,9,n)),[2 1]),'Color','k','LineWidth',1.3,'LineStyle',':')
    end
end
