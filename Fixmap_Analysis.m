...
imagesc(reshape(r,[500 500]),[-1 1])
fix.maps = reshape(r,[500 500]);fix.plot
...
[h p stats] = signrank(crit(branch_id==1),crit(branch_id==2)) % gives p = 0.045
...
figure;
subplot(1,2,1)
bar(1,mean_r_its);hold on;
errorbar(1,mean_r_its,sem_r_its,'LineWidth',2,'Color','k')
...
subplot(1,2,2)
bar(1,mean(diag(scaled)));hold on;errorbar(1,mean(diag(scaled)),std(diag(scaled))./sqrt(27),'LineWidth',2,'Color','k')
load('C:\Users\Lea\Documents\Experiments\FearCloud_Eyelab\data\midlevel\svm_analysis\20160509\subjects_rand0\result.mat')
a = result;
...
%% ttest fixation number in single pixels CS+ vs CS-
v=[];
c = 0;
for cond = [90 -90]
    for sub = unique(fix.subject)
...
fprintf('Completed Pixel \n')
for n = 1:length(M)
    [hyp(n) pval(n)] = ttest(M(n,1:26),M(n,27:end));
    if mod(n,100)==0
        fprintf('%g ...\n',n)
    end
end
figure
imagesc(mean(fix.maps(:,:,1:26)-fix.maps(:,:,27:end),3))
hold on
for n = find(pval<0.05);[y x] = ind2sub([size(fix.maps,1),size(fix.maps,2)],n);text(x,y,'x','FontSize',20);end
%% SVM CS+ vs CS-  - the numbers
...
bla = r4;
m = mean(mean(bla.result,2),3)%first over bootstr, then subjects
sd = std(mean(bla.result,2),0,3)%first over bootstr, then subjects

rcsn = 1-bla.result(8,:,:);
m = mean(mean(rcsn,2),3)%first over bootstr, then subjects
sd = std(mean(rcsn,2),0,3)%first over bootstr, then subjects
...
clear all;
%get the design matrix (same as above)
x                = [pi/4:pi/4:2*pi];
const            = ones(8);
const            = CancelDiagonals(const,NaN);
% phys = Scale([cos(x') sin(x')]*[cos(x') sin(x')]');
...
subs             = setdiff(subs,[20 22 7]);
fix              = Fixmat(subs, [2 4]);
diameter         = 10;%diameter of the search light pixels
...
for ph               = [2 4];
    for diameter         = 4:2:20;
        for fwhm             = 5:5:50;
            ...
            subc             = 0;
            betas            = nan(500*500,2);
            for subject = unique(fix.subject);%run across subjects
                subc = subc + 1;
                %creaete the query cell
                v    = [];
                c    = 0;
                ...
                for xx = 1:diameter:fix.rect(3)
                    for yy = 1:diameter:fix.rect(4)
                        %get the search light.
                ...
                %     subplot(1,2,1);imagesc(fix.stimulus);hold on;h=imagesc(reshape(nanmean(betas(:,1,:),3),[500 500]));set(h,'alphaData',Scale(reshape(nanmean(betas(:,1,:),3),[500 500]))*.9+.1 );hold off;axis image;
                %         subplot(1,2,2);imagesc(fix.stimulus);hold on;h=imagesc(reshape(nanmean(betas(:,2,:),3),[500 500]));set(h,'alphaData',Scale(reshape(nanmean(betas(:,2,:),3),[500 500]))*.9+.1 );hold off;axis image;
                %         drawnow;
            end
            b{ph,fwhm,diameter} = reshape(nanmean(betas(:,1:2,:),3),[500 500 2]);
            save ~/Dropbox/selim/Fearamy/betas.mat b;
        end
    end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
