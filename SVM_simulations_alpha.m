function [R,Rbin, AVE,result,confmats]=SVM_simulations_alpha(subjects,alpha)
%%
...
for kernel_fwhm = 10:10:80;
    ...
    [~,fix]             = evalc('Fixmat(subjects,1)');%get the data
...
for neig    = 1:teig%test different numbers of eigenvectors
    fprintf('Starting neig %d - smoothing %d at Time %s \n',neig,fwhm,datestr(now,'HH:MM:SS'))
    %
    result      = [];
    w           = [];
    confmats    = [];
    ...
    a = squeeze(mean(confmats(:,:,:,neig,fwhm),3));%mean confusionma
    ...
    Rbin(:,neig,fwhm)   = diag((a./sum(a(:)))./repmat(sum((a./sum(a(:))),2),[1,2]));%store both result for 1 and 0
    AVE(:,:,neig,fwhm)  = reshape(eigen(:,1:neig)*mean(w,2),[50 50]);%average the hyperplans
end
end
try
    save(sprintf('%shome%skampermann%sDocuments%sfearcloud%sdata%smidlevel%ssvm_analysis%sfindingparams%sSVM_simulations_alpha_NEV_%g_FWHM_%g_%s.mat',filesep,filesep,filesep,filesep,filesep,filesep,filesep,filesep,filesep,teig,fwhm,datestr(now,'ddmmyy'),'AVE','R','Rbin','confmats','result','subjects','alpha'));
catch
    keyboard
end
